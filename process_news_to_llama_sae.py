...
# -*- coding: utf-8 -*-
"""
process_news_to_sae_llama3_full_latent.py
-----------------------------------------
Build SAE representation vectors (FULL latent width) for pre-cleaned news CSVs with columns:
- Stock_symbol
- EPSDATS (YYYY-MM-DD)
...
- index_symbol_epsdats.json (global index of doc_id by (symbol, date))

Example:
python process_news_to_sae_llama3_full_latent.py \
...
def _infer_latent_dim_from_sae(sae) -> int:
    # Try common attribute paths for decoder weight
    ...
    # Some implementations store d_sae in config
    for path in ("cfg", "config"):
        ...
        for k in ("d_sae", "d_out", "n_features", "n_codes"):
            ...
            if isinstance(val, int) and val > 0:
                return int(val)
    raise RuntimeError("Could not infer latent dimension from SAE (decoder weight / config not found).")


def _encode_block(sae, activations: torch.Tensor) -> np.ndarray:
    """
    activations: [batch, seq, d] or [seq, d] on device
    returns: np array [T, D_latent] (FULL dense vector)
    """
    ...
    with torch.no_grad():
        out = sae.encode(activations)  # EleutherAI SAEs often return Top-K (indices + values)

    # If the library already returns a dense tensor under common names:
    ...
    if idx is None or vals is None:
        raise TypeError(
            f"Unexpected SAE encode() result: {type(out)}; "
            f"could not find (indices, values). Available attrs: {dir(out)}"
        )
    ...
    for ch in chunks:
        with torch.inference_mode():
            if not non_residual_capture:
                # Residual-stream path via hidden_states
            ...
            # Encode -> dense FULL latent vector per token
...
def main():
    ...
    logging.getLogger("NEWS_SAE_Pipeline").info("Wrote index_symbol_epsdats.json")
